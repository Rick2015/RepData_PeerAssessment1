...
newdata<-adata
newdata$steps[is.na(newdata$steps)]<-mean_steps[as.character(is.na(newdata$steps),3)]       
```
...
#newdata$weekday<-as.factor(ifelse(match(wday(newdata$date),2:6, nomatch=0), TRUE, FALSE))
newdata$weekend<-as.factor(ifelse(match(wday(newdata$date),c(1,7), nomatch=0), "weekend", "weekday"))
```
...
library(lattice)
new_mean_steps<-tapply(newdata$steps, newdata$interval, mean, na.rm = TRUE)
#xyplot(steps ~ interval | weekend, data=newdata, type="l", layout= c(1,2))
